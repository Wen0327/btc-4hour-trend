# BTC/ETH Signal v6 — Claude Code 接手文件
...
- **是什麼**：BTC/ETH 市場狀態儀表板 + V6 預測模型（順勢回撤 pullback-in-trend）
- **現狀**：已在 macOS launchd 排程運作，每 4h 推 Discord + 寫 CSV
- **階段**：V6 資料收集中（2026-07-13 起），預計 **2026-08-10** 回測
...
| V4 布林+確認 | 布林觸軌+KD/RSI/MACD | 回測 PF 3.32 但 **live 4 週 0 信號**（趨勢中啞巴），失敗 |
| V5 Ichi+Fib+OBV | 一目均衡主導 | 回測 52%/1.21，**live 4 週 42.3%/PF 0.71，Phase 2 未通過**。SHORT 33%（追空追在低點）、22 連虧（信號自相關） |
| **V6 順勢回撤** | 4h EMA20/50 趨勢 + 回撤觸發 + 24h 冷卻 | 回測：BTC 48%/PF 2.00（48h）、SHORT 50-59% vs V5 的 33%。低頻高盈虧比。**live 驗證中（2026-07-13 起）** |

### 已驗證無效的方向
- BTC 與外部資產（SPY、MSTR、GLD、DXY）相關性不穩定，無預測力
- ETF 流量有滯後性，對 4h/24h 預測無效
- 加權打分框架預測方向沒有 edge（V1~V3 都試過）
- 均值回歸（V4）在趨勢行情啞巴；趨勢跟隨的滯後指標（V5 Ichimoku）在盤整追高殺低
- **教訓：每 4h 無條件出信號會產生自相關的假樣本（V5 22 連虧）。V6 用冷卻機制強制信號獨立**
